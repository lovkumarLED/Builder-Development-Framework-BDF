...
function routeCard(route, store) {
  ...
  return `<article class="claude-route-card ${applied ? "claude-route-card--applied" : "claude-route-card--saved"}" data-route-id="${escapeHtml(route.id)}"><div class="claude-route-card__head"><h3>${escapeHtml(route.name)}</h3>${marker}</div><dl class="claude-route-card__meta"><div><dt>Endpoint</dt><dd class="mono">${escapeHtml(route.baseUrl)}</dd></div><div><dt>Model</dt><dd>${escapeHtml(route.model)}</dd></div><div><dt>Auth</dt><dd>${authLabel} · <span class="mono">${escapeHtml(route.secretEnvRef)}</span></dd></div></dl><div class="claude-route-card__actions">${actions}</div></article>`;
}
...
function valuesFrom(dialog) {
  return {
    ...
    autoCompactWindow: Number(dialog.querySelector("#claudeRouteCompact").value),
...
export function openRouteDetails(workspace, routeId) {
  api.claudeRoutes().then(data => {
    ...
    openDialog({
      ...
      content: `<dl class="stack"><div><dt class="eyebrow">Endpoint</dt><dd class="mono">${escapeHtml(route.baseUrl)}</dd></div><div><dt class="eyebrow">Auth</dt><dd>${escapeHtml(route.authKind)} reference: ${escapeHtml(route.secretEnvRef)}${route.envVarManaged ? ' <span class="claude-type-chip">managed by Switcher</span>' : ""}</dd></div><div><dt class="eyebrow">Model</dt><dd>${escapeHtml(route.model)}</dd></div><div><dt class="eyebrow">Policies</dt><dd>Gateway discovery ${route.gatewayDiscovery ? "on" : "off"} - Betas ${route.disableExperimentalBetas ? "disabled" : "enabled"} - Auto-compact ${escapeHtml(String(route.autoCompactWindow))} - Nonessential traffic ${route.disableNonessentialTraffic ? "disabled" : "enabled"}</dd></div><div><dt class="eyebrow">Status</dt><dd>${isApplied(route, data) ? "Applied" : hasPendingChanges(route, data) ? "Changes not applied" : "Saved"}</dd></div></dl>`,
