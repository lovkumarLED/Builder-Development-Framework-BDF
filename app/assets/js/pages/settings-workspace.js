...
function mcpRows(mcps) {
  ...
  return `<div class="settings-list">${entries.slice(0, 5).map(([name]) => `<div class="settings-list-row settings-mcp-row"><span class="settings-row-icon settings-row-icon--green">${icon.server}</span><strong>${escapeHtml(name)}</strong><span class="settings-configured"><i></i>Configured</span><button class="settings-icon-button" type="button" data-route="integrations" aria-label="Open ${escapeHtml(name)} settings">${icon.more}</button></div>`).join("")}</div>`;
}
  ...
  return `<section class="settings-workspace">
    ...
    <div class="settings-layout">
      <aside class="settings-rail">
        ...
        <section id="settingsProfile" class="settings-profile-compact" aria-labelledby="activeProfileTitle">
      ...
      <div class="settings-modules">
        <article id="modelsReasoning" class="card settings-module settings-models"><h2>Models & reasoning</h2>
        ...
        <div class="settings-side-stack">
          <article id="settingsPlugins" class="card settings-module settings-plugins-module"><h2>Plugins</h2>${pluginRows(plugins)}<button class="button settings-outline-button" type="button" data-route="integrations">${icon.plus}Add plugin</button></article>
          <article id="settingsModelManager" class="card settings-module settings-model-manager"><div class="settings-model-manager-head"><div><span>Provider library</span><h2>Manage models</h2></div><label>Provider<select id="modelManagerProvider">${providerOptions(providers)}</select></label></div><div id="modelManagerList" class="settings-model-manager-list">${modelManagerRows(provider)}</div><div class="settings-model-manager-foot"><span id="modelManagerSelection">0 selected</span><button id="modelManagerDelete" class="button settings-model-manager-delete" type="button" disabled>Delete selected</button></div><p id="modelManagerMessage" class="settings-message" role="status"></p></article>
        </div>
        <article id="settingsMcps" class="card settings-module settings-mcps-module"><h2>MCP servers</h2>${mcpRows(mcps)}<button class="button settings-outline-button" type="button" data-route="integrations">${icon.plus}Add server</button></article>
        <article id="buildOutput" class="card settings-module settings-build-module"><h2>Build output</h2><pre id="settingsBuildOutput" class="settings-terminal" aria-live="polite"><span class="terminal-ready">[ready]</span> Build has not run in this session.</pre><button id="settingsBuild" class="button button--primary" type="button">${icon.play}Build my config</button></article>
        <article id="developerSettings" class="card settings-module settings-developer-module"><h2>Developer settings</h2>
