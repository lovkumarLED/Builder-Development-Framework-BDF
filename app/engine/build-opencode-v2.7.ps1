# ============================================================
# OpenCode Configuration Builder V2.7
# ============================================================
# Author  : Love (owner)
#           ChatGPT (early planning/blueprints)
#           OpenCode / deepseek-v4-flash-free (primary implementation)
# Purpose: Generate opencode.json from modular profile files. (Builder V2.7 - JSON Schema Validation)
#
# Features
# --------
...
# - Provenance sidecar (F5): <artifact>.provenance.json stamped at generation (artifact from profiles/<profile>/target.json, default opencode.json)
...
# -> { "artifact": "opencode.json" }. Missing, unreadable or invalid
# target.json falls back to "opencode.json" (backward compatible).
$TargetArtifact = "opencode.json"
...
if (Test-Path $TargetFile) {
    ...
    try {
        ...
        Write-Host "[!] target.json unreadable or invalid - using default artifact 'opencode.json'." -ForegroundColor DarkYellow
    }
}

if (-not $TargetArtifact.EndsWith(".json")) { $TargetArtifact = "$TargetArtifact.json" }
...
function Write-Header {
    ...
    Write-Host "      OpenCode Config Builder v$BuilderVersion" -ForegroundColor Cyan
    Write-Host "==============================================" -ForegroundColor Cyan
    Write-Host ""
}
...
"@
            ...
            if ($null -ne $Entry.Value.apiKey -and ($null -eq $Entry.Value.options -or $null -eq $Entry.Value.options.apiKey)) {
                ...
                if ($null -eq $Entry.Value.options) {
                    ...
                    $Entry.Value.options = [ordered]@{}
                }

                $Entry.Value.options.apiKey = $Entry.Value.apiKey
...
function Merge-Models {
    ...
    foreach ($ProviderName in $ProviderRoot.Keys) {
        ...
        if ($GlobalModels -and $GlobalModels.models) {

            Assert-NoDuplicateModelNames $GlobalModels "models.json"
...
function Verify-Plugins {
    ...
    if ($Plugins -and @($Plugins.plugin).Count -gt 0 -and -not $Final.plugin) {
...
function Verify-Mcp {
    ...
    if ($MCP -and @($MCP.mcp.PSObject.Properties).Count -gt 0 -and -not $Final.mcp) {
...
if ($Models)   { Write-Success "models.json"   } else { Write-Warning "models.json missing - models section will be skipped" }
...
if ($DroppedProviders.Count -gt 0) {
    ...
    foreach ($P in $DroppedProviders) {
        ...
        Write-Warning "Provider '$P': models not found (no <provider>-models.json, providers/$P/models.json, inline, or global models.json). Provider will not be considered active and was removed from settings.json."
    }
...
if ($WhatIf) {
    ...
    Write-Success "Builder V2.7 dry-run finished successfully."
...
Write-Success "Builder V2.7 finished successfully."
