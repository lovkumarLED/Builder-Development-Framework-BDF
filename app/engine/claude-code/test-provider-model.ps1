...
Run-Test 'G3-5 pipeline alias sorting without providers' 'G3-5' {
    $root=$null;try{$root=New-Pipeline {$args[0].env|Add-Member ANTHROPIC_DEFAULT_SONNET_MODEL 'gateway/pin/sonnet';$args[0].env|Add-Member ANTHROPIC_DEFAULT_HAIKU_MODEL 'gateway/pin/haiku'} {$args[0].envPolicy.gatewayDiscovery=$false;$args[0].envPolicy.disableNonessentialTraffic=$false};$r=Invoke-Inspector $root ($LoopbackPrefix+':9');$raw=[IO.File]::ReadAllText($r.Path);$o=$raw|ConvertFrom-Json;Assert-True (($o.aliasPins.alias-join ',')-ceq 'haiku,sonnet') 'alias order';Assert-True ($o.models.Count-eq 0) 'pins became models';foreach($name in @('providers','providerList','activeProviders')){Assert-True ($null-eq $o.PSObject.Properties[$name]) 'provider property emitted'}}finally{Remove-Root $root}
}
