# ============================================================
# KiloCode Configuration Builder V1 (K1) - Test Harness
# ============================================================
# Author  : Love + ChatGPT
...
# 18. Schema: provider file violation fails
# 19. Schema: models file violation fails
# 20. Schema: no schema dir warns + continues
# 21. Pre-flight: missing provider file aborts
# 22. -WhatIf: nothing written, exit 0
# 23. -Doctor: clean config exits 0
# 24. -Doctor: corrupt config exits 1
# 25. Backup retention: -KeepBackups honored
# 26. Provenance: sidecar fields + sha correct
# 27. Diff summary: Added/Removed lines; identical input silent
# 28. Builder spec covers V2.7 tokens (F1-F7)
# New (LSP group):
# 29. LSP enabled true -> generated lsp key
# 30. LSP enabled object -> lsp object round-trips
# 31. LSP disabled omits the lsp section
# 32. No lsp.json omits the lsp section
# 33. LSP false value omits the lsp section
# ============================================================

$BuilderPath = Join-Path $PSScriptRoot "build-kilo-v1.ps1"

$TestRootBase = Join-Path $env:TEMP "kilo-builder-tests"
...
function Read-Generated {
    ...
    $Generated = Join-Path $Root "kilo.json"

    if (!(Test-Path $Generated)) {

        throw "kilo.json was not generated."
    }
...
# ------------------------------------------------------------
# Invoke-Builder adapted for build-kilo-v1.ps1
...
function Write-ProfileSettings {
    ...
    if ($WithSchema) { $Settings['$schema'] = "https://app.kilo.ai/config.json" }
...
function Test-AllProvidersDiscovered {
    ...
    try {
        ...
        # Per-provider models (the only source the builder reads since the
        # global models.json lookup was removed).
        Write-ProfileProviderModels $Root "default" "modal" @{
            "m-1" = @{ name = "Model One" }
        }
        Write-ProfileProviderModels $Root "default" "omniroute" @{
            "m-2" = @{ name = "Model Two" }
        }
...
function Test-MalformedProviderFails {
    ...
    try {
        ...
        Assert-True (!(Test-Path (Join-Path $Root "kilo.json"))) `
            "kilo.json should not be written after a failed build."
    }
    finally {
...
function Test-NonActiveProfileModelsIgnored {
    ...
    try {
        ...
        $Raw = Get-Content (Join-Path $Root "kilo.json") -Raw
...
function Test-SettingsPersistRoundTrip {
    ...
    try {
        ...
        Write-ProfileProviderModels $Root "default" "modal" @{ "m-1" = @{ name = "Model One" } }
        Write-ProfileProviderModels $Root "default" "omniroute" @{ "m-2" = @{ name = "Model Two" } }
        ...
        Assert-True ($Settings.'$schema' -eq "https://app.kilo.ai/config.json") `
...
# ------------------------------------------------------------
# Test 12 - Real BUILDER_SPEC_KILO_ADAPTER.md covers the V2.5 tokens
...
function Test-BuilderSpecCoversV25 {
    ...
    $SpecPath = Join-Path (Split-Path $PSScriptRoot -Parent) "docs\BUILDER_SPEC_KILO_ADAPTER.md"

    if (-not (Test-Path $SpecPath)) {

        Write-Host "[i] BUILDER_SPEC_KILO_ADAPTER.md not present - skipping spec token coverage (project doc optional)."
        return
    }
    ...
    foreach ($Token in @(
        ...
        Assert-True $Spec.Contains($Token) "BUILDER_SPEC_KILO_ADAPTER.md is missing the token: $Token"
    }
}
...
function Assert-GeneratedExists {
    ...
    if ($ShouldExist -and -not $Exists) { throw "kilo.json was not generated." }
    if (-not $ShouldExist -and $Exists) { throw "kilo.json should NOT have been generated." }
}
...
function Test-SchemaValidPasses {
    ...
    try {
        ...
        Write-ProfileSettings $Root -Active @("omniroute")
        Write-ValidProvider $Root "omniroute" "OmniRoute"
        Write-ProfileProviderModels $Root "default" "omniroute" @{ "m-1" = @{ name = "Model One" } }
...
function Test-WhatIfWritesNothing {
    ...
    try {
        ...
        Assert-True ($Run.Output.Contains("Would write kilo.json")) `
            "Expected the dry-run notice. Output: $($Run.Output)"

        Assert-GeneratedExists $Root $false
        Assert-True (!(Test-Path (Join-Path $Root "kilo.provenance.json"))) `
            "WhatIf must not write the provenance sidecar."

        $Backups = @(Get-ChildItem -Path (Join-Path $Root "backup") -Filter "kilo_*.json" -File -ErrorAction SilentlyContinue)
...
function Test-BackupRetentionHonordsKeepBackups {
    ...
    try {
        ...
        $Backups = @(Get-ChildItem -Path (Join-Path $Root "backup") -Filter "kilo_*.json" -File)
...
function Test-ProvenanceSidecarCorrect {
    ...
    try {
        ...
        $Prov = Get-Content (Join-Path $Root "kilo.provenance.json") -Raw | ConvertFrom-Json

        Assert-True ($Prov.builderVersion -eq "K1") "Wrong builderVersion: $($Prov.builderVersion)"
...
# ------------------------------------------------------------
# Test 28 - Real BUILDER_SPEC_KILO_ADAPTER.md covers the V2.7 tokens (F1-F7)
# ------------------------------------------------------------
...
function Test-DynamicTargetArtifact {
    ...
    try {
        ...
        Write-ValidProvider $Root "omniroute" "OmniRoute"
        Write-ProfileProviderModels $Root "alt" "omniroute" @{ "m-1" = @{ name = "Model One" } }

        # Non-kilo artifact proves config-driven resolution.
        ...
        Assert-True (!(Test-Path (Join-Path $Root "kilo.jsonc"))) `
            "Default 'kilo.jsonc' must not be generated for a dynamic target."
        ...
        $LegacyBackups = @(Get-ChildItem -Path (Join-Path $Root "backup") -Filter "kilo_*.json" -File -ErrorAction SilentlyContinue)

        Assert-True ($LegacyBackups.Count -eq 0) "Backup must NOT use the legacy 'kilo_*' prefix."
...
# ------------------------------------------------------------
# Test 31 - Dual-key normalization mirrors apiKey into options.apiKey
# ------------------------------------------------------------

function Test-DualKeyOptionsMirrored {
    ...
    try {
        ...
        # Hand-written style: only the top-level apiKey (no options.apiKey).
        Write-JsonFile $Root "providers\omniroute.json" @'
{
    "id": "omniroute",
    "provider": {
...
'@
        ...
        Assert-True ($Generated.provider.omniroute.options.apiKey -eq "{env:OMNIROUTE_API_KEY_TEST}") `
            "options.apiKey must be mirrored from apiKey. Got: $($Generated.provider.omniroute.options.apiKey)"
    }
    finally {
...
# ------------------------------------------------------------
# LSP group: lsp.json -> generated lsp key
# ------------------------------------------------------------
...
Write-Host "   KiloCode Config Builder V1 (K1) Test Harness" -ForegroundColor Cyan
Write-Host "==============================================" -ForegroundColor Cyan
Write-Host ""
