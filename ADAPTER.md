...
profiles/default/models.json     (optional)
profiles/default/plugins.json    (optional)
profiles/default/mcp.json        (optional)
providers/omniroute.json         (provider definition)
```

Additional profiles (`coding`, `experimental`, `minimal`) contain only `settings.json` and contribute their provider selection to the build.
...
The complete model list is defined in:
