...
# ðŸ”¥ Builder Development Framework (BDF) + AI Switcher App
...
4. **Seed** the three profiles â€” `coding` (the main) + `experimental` +
   ...
   `mcp.json`/`plugins.json` are user-owned after creation â€” the framework
...
(settings â†’ providers â†’ models â†’ plugins â†’ mcp) â†’ output verification â†’
...
## ðŸ¤– How the AI Switcher app works
### See it in action
...
```
docs/
â”œâ”€â”€ app/                    â† the AI Switcher app (self-contained)
...
| Phase 14 â€” GUI App (AI Switcher) | âœ… |
