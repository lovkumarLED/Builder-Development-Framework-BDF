...
> SUPERSEDED — V2.7 released as project version 2.5.0 (2026-08-06); Task 10 rehearsal
> executed in session 21; P1 env-key policy replaced the modal.json live-key note
> (providers carry `{env:VAR}` placeholders only since session 20). Historical record.

## STATUS: Tasks 1-8 COMPLETE, Task 9 awaiting USER REVIEW, Task 10 pending

## Done this session (2026-08-06)
- All 3 harnesses green: 17/17 (V2.1) + 13/13 (V2.5) + 28/28 (V2.7). Deterministic. (Later sessions grew the V2.7 harness to 31/31.)
...
- providers/modal.json RESTORED from backup at the time (contained a live API key — do not commit to public remotes; user decision). LATER SUPERSEDED by the P1 env-key policy (session 20): modal.json recreated with `{env:MODAL_API_KEY_OPENCODE}` placeholder only, and the P1 gate asserts no literal keys in any generated output.
